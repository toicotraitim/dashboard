...
@section('content')
    ...
    <div class="card card-danger mx-lg-5">
      ...
      <!-- /.card-header -->
        ...
        <form role="form" method="POST" action="{{ route('product-management.destroy',['product_management' => $product['id']]) }}" enctype="multipart/form-data">
            @method("DELETE")
            {{ csrf_field() }}
            <div class="card-body">
                @if (session('success'))
                ...
                <p> Bạn có chắc chắn muốn xóa chuyên mục <b> {{ $product['name'] }} </b>  không? </p>
